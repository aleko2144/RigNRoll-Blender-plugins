...
def read_wdb_data(context, filepath, use_some_setting):
	...
	for i in range(len(sections)):
		...
		if sections[i][5] == 309 or sections[i][5] == 310: #312, указанный в начале файла, обычно равен 309
			...
			if (vertices_len > 999990):
				...
				if (verts_type == 18):
					...
					for i in range(vertices_len):
						...
						fnx = struct.unpack('<f',file.read(4))[0]
						fny = struct.unpack('<f',file.read(4))[0]
						fnz = struct.unpack('<f',file.read(4))[0]
						
						vertices.append((x, y, z))
						vert_normals.append((fnx, fny, fnz))
						uvs.append((u, v))													
					...
					for i in range(vertices_len):
						...
						uv3 = struct.unpack('<f',file.read(4))[0]
			...
			if (verts_type == 4370) or (verts_type == 4374) or (verts_type == 4434) or (verts_type == 4626) or (verts_type == 5202) or (verts_type == 3420930) or (verts_type == 5510162) or (verts_type == 5510166):
				for i in range(vertices_len):
					index = []
					bindex = bindexes[i]
					index.append(i)
